  ...
  return (
    <div className="rounded-2xl bg-[var(--card)] border border-[var(--border)] p-5 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <span className="text-3xl font-bold text-[#F4A90B]">87.1 %</span>
      ...
      <div className="mt-6 flex h-24 items-end gap-2">
        {barHeights.map((h, i) => (
          <div key={i} className="flex-1 rounded-sm bg-[#F4A90B]" style={{ height: `${h}%` }} />
        ))}
      </div>
      <div className="mt-3 flex items-center justify-between">
        <span className="flex items-center gap-1 rounded border border-[#F4A90B]/30 bg-[#F4A90B]/10 px-2 py-0.5 text-xs text-[#F4A90B]">
          <TrendingDown className="h-3 w-3" /> 7.5%
        </span>
        <span className="text-xs text-[var(--text-muted)]">in last 7 Days</span>
      </div>
    </div>
  )
}