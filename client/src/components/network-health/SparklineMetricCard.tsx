...
export function SparklineMetricCard({ title, value, unit, change, changeType, sparklineData, sparklineColor }: SparklineMetricCardProps) {
  const badgeBg = changeType === 'positive' ? 'bg-emerald-500' : 'bg-red-600'
  return (
    <div className="flex h-[125px] flex-col gap-2 rounded-2xl bg-[var(--card)] border border-[var(--border)] p-4 shadow-sm">
