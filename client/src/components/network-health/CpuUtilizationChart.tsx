...
const data = [
  { time: 'Sat 18:00', cpu: 35 }, { time: 'Sun 00:00', cpu: 42 },
  { time: 'Sun 06:00', cpu: 58 }, { time: 'Sun 12:00', cpu: 72 },
  ...
  return (
    <div className="rounded-2xl bg-[var(--card)] border border-[var(--border)] p-5 shadow-sm">
      <div className="mb-1 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-[var(--text)]">CPU Utilization for localhost</h3>
          <p className="text-sm text-[var(--text-muted)]">Datasource: user</p>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" className="flex items-center gap-1 rounded-xl bg-[var(--card-alt)] border border-[var(--border)] px-3 py-2 text-sm text-[var(--text)] hover:bg-[var(--hover)]">
            24 hours <ChevronDown className="h-4 w-4" />
          </button>
      ...
      <div className="h-56 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            <YAxis axisLine={false} tickLine={false} tick={{ fill: 'var(--chart-text)', fontSize: 11 }} domain={[0, 80]} />
            ...
            <Area type="monotone" dataKey="cpu" stroke="#F4A90B" strokeWidth={2} fill="url(#cpuGradient)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <div className="mt-4 flex flex-wrap gap-4">
        {categories.map((cat) => <span key={cat} className="text-xs text-[var(--text-muted)]">{cat}</span>)}
      </div>
    </div>
  )
}
