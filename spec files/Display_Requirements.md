...
**How services map to plugin groups:**

Each `ServiceStatus` row stores the Nagios command name reported by
`statusjson.cgi` (`Check_Command`, arguments stripped). The backend resolves
that command to its check plugin:

- `pinpoint_nd_<plugin>` (generated by Network Discovery) → the plugin's
  `check_plugin` in `network_discovery/plugin_registry.py`, e.g.
  `pinpoint_nd_snmp` → `check_snmp`, `pinpoint_nd_https` → `check_http`
- `pinpoint_<plugin>` (generated by Plugin Manager) → `<plugin>`, e.g.
  `pinpoint_check_dig` → `check_dig`
- `check_local_<plugin>` (stock Nagios localhost commands) → `check_<plugin>`
...
resolves to `check_ncpa` and is grouped under the **NCPA** display name.
...
**The plugin registry is the single source of truth** for plugin → command
mappings. Both `create_host_cfg.py` (which generates Nagios config) and
`statistics.py` (which groups services for display) read from
`network_discovery/plugin_registry.py`. Adding a new service type — including
one with several metrics — requires only a new `PluginDefinition` there.
