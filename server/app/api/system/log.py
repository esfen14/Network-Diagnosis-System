...
def activity_logs():
    try:
        ...
        query = (
            ...
            .join(
                User,
                User.UserID == ActivityLog.UserID
            )
        )
...
def configuration_change_logs():
    try:
        ...
        if search:
            query = query.where(
                sa.or_(
                    ConfigurationChanges.Conf_Type.ilike(
                        f"%{search}%"
                    ),
                    ConfigurationChanges.Parameter_Name.ilike(
                        f"%{search}%"
                    ),
                    ConfigurationChanges.Old_Value.ilike(
                        f"%{search}%"
                    ),
                    ConfigurationChanges.New_Value.ilike(
                        f"%{search}%"
                    ),
                    User.First_Name.ilike(
                        f"%{search}%"
        ...
        if start_date:
            query = query.where(
                sa.cast(
                    ConfigurationChanges.Changed_At,
                    sa.Date
                ) >= start_date
            )
        ...
        for config, activity, user in logs.items:
            ...
            items.append({
                ...
                "description": (
                    ...
                    f"from '{config.Old_Value}' "
                    f"to '{config.New_Value}'."
                ),
                "timestamp": (
                    config.Changed_At.isoformat()
                ),
                "tagId": (
                    f"CC-{config.ConfChangesID:04d}"
                ),
                "details": {
...
def network_discovery_logs():
    try:
        ...
        if search:
            query = query.where(
                sa.or_(
                    NetworkDiscoveryStatus.Message.ilike(
                        f"%{search}%"
                    ),
                    NetworkDiscoveryStatus.Error.ilike(
                        f"%{search}%"
                    ),
                    User.First_Name.ilike(
                        f"%{search}%"
        ...
        if start_date:
            query = query.where(
                sa.cast(
                    NetworkDiscoveryStatus.Start_At,
                    sa.Date
                ) >= start_date
            )
        ...
        for discovery, activity, user in logs.items:
            ...
            items.append({
                ...
                "timestamp": (
                    discovery.Start_At.isoformat()
                ),
                "tagId": (
                    f"ND-{discovery.DiscoveryStatusID:04d}"
                ),
                "details": {
                    "status": status,
                    ...
                    "start_at": (
                        discovery.Start_At.isoformat()
                        if discovery.Start_At
                        else None
                    ),
                    "completed_at": (
                        discovery.Completed_At.isoformat()
                        if discovery.Completed_At
                        else None
                    ),
...
def ncpa_deployment_logs():
    try:
        ...
        if search:
            query = query.where(
                sa.or_(
                    NCPADeploymentStatus.Message.ilike(
                        f"%{search}%"
                    ),
                    NCPADeploymentStatus.Error.ilike(
                        f"%{search}%"
                    ),
                    User.First_Name.ilike(
                        f"%{search}%"
        ...
        for deployment, activity, user in logs.items:
            ...
            items.append({
                ...
                "timestamp": (
                    deployment.Start_At.isoformat()
                ),
                "tagId": (
                    f"NP-{deployment.NCPADeployStatusID:04d}"
                ),
                "details": {
                    "status": status,
...
def export_logs():
    try:
        ...
        for export, activity, user in logs.items:
            ...
            export_format = (
                export.Export_Format.value
                if export.Export_Format
                else "Unknown"
            )
            ...
            items.append({
                ...
                "description": (
                    f"Exported {export.Report_Type} "
                    f"report as "
                    f"{export_format.upper()}."
                ),
                "timestamp": (
                    export.Exported_At.isoformat()
                ),
                "tagId": (
                    f"EX-{export.ExportID:04d}"
                ),
                "details": {
                    "report_type": export.Report_Type,
                    "format": export_format,
                    "start_date": (
                        export.Start_Date.isoformat()
                        if export.Start_Date
                        else None
                    ),
                    "end_date": (
                        export.End_Date.isoformat()
                        if export.End_Date
                        else None
                    ),
                    "exported_at": (
