...
# check_local_<plugin> (check_local_load, check_local_disk, ...).
LOCAL_COMMAND_PREFIX = "check_local_"
...
def _plugin_key(service_name: str, check_command: Optional[str] = None) -> str:
    """
    Resolve a service to the check plugin it runs (e.g. "check_snmp").

    Preferred: the Nagios command name recorded in ServiceStatus.Check_Command
    (reported by statusjson.cgi), which links a service to its plugin without
    relying on how the service happens to be named:

        "pinpoint_nd_snmp"     →  check_snmp   (Network Discovery, via plugin_registry)
        "pinpoint_check_dig"   →  check_dig    (Plugin Manager)
        "check_local_load"     →  check_load   (stock Nagios localhost commands)
        "check_ping"           →  check_ping   (any other command, as-is)

    Fallback for rows without a Check_Command (recorded before it existed):
    ...
    if command:
        ...
        if command.startswith(DISCOVERY_COMMAND_PREFIX):
            return f"check_{command[len(DISCOVERY_COMMAND_PREFIX):]}"
        if command.startswith(PLUGIN_MANAGER_COMMAND_PREFIX):
            return command[len(PLUGIN_MANAGER_COMMAND_PREFIX):]
        if command.startswith(LOCAL_COMMAND_PREFIX):
            return f"check_{command[len(LOCAL_COMMAND_PREFIX):]}"
        return command

    dash_prefix = service_name.split("-")[0].lower().strip()
    prefix = dash_prefix.split("_")[0]
    definition = find_plugin_by_name_or_alias(dash_prefix) or find_plugin_by_name_or_alias(prefix)
    return definition.check_plugin if definition else f"check_{prefix}"
...
def ncpa_averages(
    ...
    """
    ncpa_svcs = [s for s in services if _plugin_key(s.Service, s.Check_Command) == "check_ncpa"]
...
def service_health_by_plugin(services: list[ServiceStatus]) -> list[dict]:
    ...
    for svc in services:
        key = _plugin_key(svc.Service, svc.Check_Command)
...
def _find_service_by_plugin(
    ...
    for name, service in services_by_name.items():
        if _plugin_key(name, service.Check_Command) == plugin_key:
            return service
    return None
